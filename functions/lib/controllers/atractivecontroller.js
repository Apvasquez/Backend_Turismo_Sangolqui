...
exports.lisAtractives = exports.createAtractives = void 0;
const index_1 = require("../index");
const message_1 = require("../models/message");
const atractive_1 = require("../models/atractive");
const collection = "atractive";
//--------------------------------------------------------------------------------------------------------////
//------------------------------------------------Clients CRUD`s------------------------------------------////
//--------------------------------------------------------------------------------------------------------////
async function createAtractives(req, res) {
    try {
        const newAtractive = atractive_1.atractive(req.body);
        const AtractiveAdded = await (await index_1.db.collection(collection).add(newAtractive));
        return res.status(201).json(message_1.Message('Atractive added', `Atractive was added with id: ${AtractiveAdded.id}`, 'success'));
    }
    catch (err) {
        return handleError(res, err);
    }
}
exports.createAtractives = createAtractives;
/*
//Search a doc by id
...
*/
async function lisAtractives(req, res) {
    try {
        let page = parseInt(req.params.page);
        let limit = parseInt(req.params.limit);
        let avoid = page == 1 ? 0 : (page - 1) * limit;
        let snapshot = await index_1.db.collection(collection).orderBy('name').offset(avoid).limit(limit).get();
        return res.status(200).json(snapshot.docs.map(doc => atractive_1.atractive(doc.data(), doc.id)));
    }
    catch (err) {
        return handleError(res, err);
    }
}
exports.lisAtractives = lisAtractives;
//list codes by interval
function handleError(res, err) {
    res.status(500).send({ message: `${err.code} - ${err.message}` });
}
//# sourceMappingURL=atractivecontroller.js.map