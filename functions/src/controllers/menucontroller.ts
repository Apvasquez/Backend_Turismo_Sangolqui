...
import { Items_Menu } from '../models/Items_Menu';
...
//------------------------------------------------Clients CRUD`s------------------------------------------////
//--------------------------------------------------------------------------------------------------------////
export async function createItems_Menu(req:Request, res: Response){
    try{
        const newItems_Menu = Items_Menu(req.body); 
        const Items_MenuAdded = await (await db.collection(collection).add(newItems_Menu));         
        return res.status(201).json(Message('Menu added', `Menu was added with id: ${Items_MenuAdded.id}`, 'success'));
    }catch(err){
...
*/
export async function lisItemsMenu(req:Request, res: Response){
    try{
        let page = parseInt(req.params.page);
        let limit = parseInt(req.params.limit);
        let avoid = page == 1 ? 0 : (page - 1) * limit;
        let snapshot = await db.collection(collection).orderBy('name').offset(avoid).limit(limit).get();
        return res.status(200).json(snapshot.docs.map(doc => Items_Menu(doc.data(), doc.id)));
    }catch(err){
        return handleError(res, err);
    }
