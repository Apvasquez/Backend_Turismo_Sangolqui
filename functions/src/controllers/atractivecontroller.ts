...
import { atractive } from '../models/atractive';
...
//------------------------------------------------Clients CRUD`s------------------------------------------////
//--------------------------------------------------------------------------------------------------------////
export async function createAtractives(req:Request, res: Response){
    try{
        const newAtractive = atractive(req.body); 
        const AtractiveAdded = await (await db.collection(collection).add(newAtractive));         
        return res.status(201).json(Message('Atractive added', `Atractive was added with id: ${AtractiveAdded.id}`, 'success'));
    }catch(err){
...
*/
export async function lisAtractives(req:Request, res: Response){
    try{
        let page = parseInt(req.params.page);
        let limit = parseInt(req.params.limit);
        let avoid = page == 1 ? 0 : (page - 1) * limit;
        let snapshot = await db.collection(collection).orderBy('name').offset(avoid).limit(limit).get();
        return res.status(200).json(snapshot.docs.map(doc => atractive(doc.data(), doc.id)));
    }catch(err){
        return handleError(res, err);
    }
